...
<html>
  <head>
    ...
    <title>采购处理详情</title><%=request.getSession().getAttribute("username") %>，您好
  					...
  					<div id="op">
  						...
  						<ol id="option">
  							<table class="hovertable" >
  									<thead><tr><td>订货日期</td><td>采购订单号</td><td>供应商名称</td><td>商品名称</td><td>条形码</td><td>货号</td><td>事物特性</td><td>数量</td><td>单价</td><td>运费</td><td>总价</td><td>备注</td><td>快递公司</td><td>快递单号</td><td>实到数量</td><td>是否到错</td><td>状态</td><td>到货时间</td><td>拆收人</td><td>发货组签收人</td><td>备注2</td><td>完好情况</td></tr></thead>
  									<s:iterator value="orderList" id="orderList" >
  										<tr>
											...
											<td><s:property value="#orderList.arrivalnum"/></td>
											<td><s:property value="#orderList.mistake"/></td>
											<td><s:property value="#orderList.condition"/></td>
											<td><s:property value="#orderList.arrivaltime"/></td>
											<td><s:property value="#orderList.openman"/></td>
											<td><s:property value="#orderList.signman"/></td>
											<td><s:property value="#orderList.remark2"/></td>
  						...
  						<table class="hovertable" >
  							...
  							<tr><td>
  							<input type="radio" name="cusOrderback.purresult" checked="checked" value="200" />申请退款
  							<input type="radio" name="cusOrderback.purresult"  value="201" />补货
  							<input type="hidden" name="cusOrderback.id"  id="cusOrderback.id" value="<s:property value="#orderList.id"/>"/>
  							</td></tr>
  						</table>
  						</ol>
  						<ol id="option">
  						...
  						<input type="hidden" name="variable" id="variable" value="purresult" />
  					</div>
  				</form>
  			</div>
  		</div>
